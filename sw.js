const CACHE = "christianmanual-v259";
...
async function _arm(d) {
  ...
  if (delay <= 0) {
    ...
    await self.registration.showNotification(d.notifTitle || '📅 Christian Manual', _notifOpts(d)).catch(() => {});
...
self.addEventListener("push", e => {
  ...
  e.waitUntil(
    self.registration.showNotification(payload.title || "📅 Christian Manual", {
